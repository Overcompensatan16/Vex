...
class ReflexOrchestrator:
    ...
    def _build_context(self, event: Any) -> ReflexContext:
        ...
        analgesia = float(self.registry.get("system.analgesia.level", 0.0) or 0.0)
        anesthesia = bool(self.registry.get("system.anesthesia.engaged", False))
    ...
    def _handle_descend(self, event: DescendEvent) -> None:
        ...
        if mode == "gain":
            ...
            return
        if mode == "analgesia":
            self.registry.set(event.t, "system.analgesia.level", event.gain)
            return
        if mode == "anesthesia":
            self.registry.set(event.t, "system.anesthesia.engaged", bool(event.gain))
            return
        region_state[mode] = event.gain

    def _arcs_for_event(self, event: Any, context: ReflexContext) -> List[ReflexArc]:
        arcs: List[ReflexArc] = []
    ...
    def _activate_rule(self, rule: ReflexRule, context: ReflexContext) -> None:
        ...
        for signal in rule.signals:
            ...
            self.registry.set(
                ...
                path,
                value,
                context={"rule": rule.name, "phase": "on"},
            )
    ...
    def _restore_signal(self, time_ms: float, path: str, baseline: Any, rule_name: str) -> None:
        self.registry.set(
            ...
            path,
            baseline,
            context={"rule": rule_name, "phase": "off"},
        )
