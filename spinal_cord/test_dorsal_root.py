# E:\Vex\spinal_cord\test_dorsal_root.py
...
def test_symbolic_router_records_events_and_ordering(fresh_modules):
    ...
    expected_adelta_delay = distance_m / dorsal_root.FIBER_VELOCITIES_M_PER_S["Aδ"] * 1000.0
    ...
    # Updated expectations: per spike -> 1 'afferent_event' + 3 'spinal_ascending'
    event_types = [etype for etype, _ in audit_logger.events]
    expected_types = []
    for _ in thalamic_fibers:
        expected_types.extend(
            ["afferent_event", "spinal_ascending", "spinal_ascending", "spinal_ascending"]
        )
    assert event_types == expected_types

    # Validate payloads only for the afferent_event entries
    afferent_payloads = [data for etype, data in audit_logger.events if etype == "afferent_event"]
    assert len(afferent_payloads) == len(thalamic_fibers)
    for payload, expected_fiber in zip(afferent_payloads, thalamic_fibers):
...
def test_jitter_introduces_gaussian_variation(fresh_modules):
    ...
    # One Gaussian sample used
    assert len(rng.samples) == 1
    # Arrival time should be delayed by base delay plus sampled jitter (clipped at >= 0)
    base_delay = (10.0 / 100.0) / dorsal_root.FIBER_VELOCITIES_M_PER_S["Aβ"] * 1000.0
    arrival_time = events[0][0]
    assert arrival_time == pytest.approx(max(0.0, base_delay + rng.samples[0]))
