...
def _register_core_paths(registry: SignalRegistry) -> None:
    registry.register("system.analgesia.level", {"range": (0.0, 1.0), "baseline": 0.0, "units": "au"})
    registry.register("system.anesthesia.engaged", {"baseline": False})

    for region in ("right_arm", "left_arm"):
        registry.register(f"{region}.biceps.contraction", {"range": (0.0, 1.0), "baseline": 0.0, "units": "au"})
        registry.register(f"{region}.biceps.activation", {"range": (0.0, 1.0), "baseline": 0.0, "units": "au"})
        registry.register(f"{region}.biceps.inhibition", {"range": (0.0, 1.0), "baseline": 0.0, "units": "au"})
        registry.register(f"{region}.triceps.activation", {"range": (0.0, 1.0), "baseline": 0.0, "units": "au"})
        registry.register(f"{region}.triceps.inhibition", {"range": (0.0, 1.0), "baseline": 0.0, "units": "au"})
        registry.register(f"{region}.elbow.flexion_intent", {"range": (-1.0, 1.0), "baseline": 0.0, "units": "au"})
        registry.register(f"{region}.elbow.extension_intent", {"range": (-1.0, 1.0), "baseline": 0.0, "units": "au"})
...
def test_withdrawal_reflex_schedules_drives_and_signals(
    ...
    updates: List[Tuple[float, str, Any, Any]] = []
    ...
    on_updates = [u for u in updates if u[3]["phase"] == "on"]
    off_updates = [u for u in updates if u[3]["phase"] == "off"]
    assert any(path.endswith("biceps.contraction") and value > 0.0 for _, path, value, _ in on_updates)
    assert all(value == 0.0 for _, _, value, _ in off_updates)
...
def test_analgesia_blocks_low_weight_withdrawal(
    ...
    registry.set(0.0, "system.analgesia.level", 0.8)
...
def test_anesthesia_gates_reflexes(
    ...
    registry.set(0.0, "system.anesthesia.engaged", True)
    event = DorsalHornReflex(t=0.0, region="right_arm", label="withdrawal", fiber="Aδ", weight=0.9)
    orchestrator.receive(event)
