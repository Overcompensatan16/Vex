...
from typing import Any, Dict, Iterator, List, MutableMapping, Optional, Tuple, Mapping, Callable
...
_REFLEX_PATH = pathlib.Path(r"E:\VexSignalRegistry\reflex_signals\reflex_signal_map.json")


_META_KEYS = {"type", "units", "baseline", "range", "latency", "location", "neurotransmitter"}


def _flatten_registry(tree: Any, prefix: str = "") -> Iterator[Tuple[str, Dict[str, Any]]]:
    if not isinstance(tree, dict):
        raise TypeError("Reflex registry must be a mapping of signal metadata")

    for key, value in tree.items():
        if not isinstance(key, str):
            raise TypeError("Registry keys must be strings")
        path = f"{prefix}.{key}" if prefix else key
        if isinstance(value, dict) and any(k in value for k in _META_KEYS):
            yield path, value
            continue
        if isinstance(value, dict):
            yield from _flatten_registry(value, path)
            continue
        raise TypeError(f"Invalid registry entry for {path!r}: expected mapping of metadata")


with _REFLEX_PATH.open(encoding="utf-8") as f:
    REFLEX_REGISTRY = {path: meta for path, meta in _flatten_registry(json.load(f))}
