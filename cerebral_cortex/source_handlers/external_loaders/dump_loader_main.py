...
from cerebral_cortex.source_handlers.external_loaders.wiki_handler import download_latest_dump
from cerebral_cortex.source_handlers.external_loaders.arxiv_handler import fetch_all_subjects


from cerebral_cortex.source_handlers.external_loaders.batch_scheduler import BATCH_SIZE, BATCHES_PER_CYCLE, batch_scheduler
...
def main(
    ...
    # Optional seeding of arXiv entries
    arxiv_records = fetch_all_subjects(max_results=5, dump_base=DEFAULT_DUMP_BASE)
    if arxiv_records:
        store_facts(arxiv_records)
        msg = f"[DumpLoader] Seeded {len(arxiv_records)} arXiv records"
        for logger in DL_LOGGERS.values():
            logger.log_event("downloader", {"message": msg})
        print(msg)
