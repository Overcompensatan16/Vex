...
# Explicit path to subject parsers; do not rely on relative locations.
...
if not os.path.isdir(SUBJECT_PARSER_DIR):
    raise FileNotFoundError(f"Subject parser directory not found: {SUBJECT_PARSER_DIR}")
...
def process_text(
    text: str, log, router: FactRouter
) -> Tuple[Dict[str, float], Dict[str, Dict[str, int]], int, List[str]]:
...
def main():
    ...
    # Persist result statistics and process details
