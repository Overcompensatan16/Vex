...
# Ensure repository root is on ``sys.path`` so in-repo modules can be imported
ROOT_DIR = os.path.normpath(os.path.join(os.path.dirname(__file__), ".."))
if ROOT_DIR not in sys.path:
    sys.path.insert(0, ROOT_DIR)

# Set up the base directory for memory stores
DEFAULT_MEMORY_PATH = os.path.normpath(r"E:\\AI_Memory_Stores")
BASE_MEMORY_PATH = os.path.normpath(os.environ.setdefault("MEMORY_BASE", DEFAULT_MEMORY_PATH))
...
SUBJECT_PARSER_DIR = os.path.normpath(
    os.path.join(ROOT_DIR, "cerebral_cortex", "temporal_lobe", "subject_parsers")
)
...
opinion_markers_node = next(
    (
        n for n in tree.body
        if isinstance(n, ast.Assign) and any(
            isinstance(t, ast.Name) and t.id == "OPINION_MARKERS" for t in n.targets
        )
    ),
    None,
)

compiled = compile(ast.Module(body=[is_opinion_node, opinion_markers_node], type_ignores=[]), filename="<ast>", mode="exec")
namespace = {}
exec(compiled, namespace)
_is_opinion_func = namespace["is_opinion"]


def _load_parser(module_name: str, func_name: str) -> Callable:
...
PARSER_FUNCS = {
    "bio": _load_parser("bio_parser", "parse_bio_text"),
    "chemistry": _load_parser("chemistry_parser", "parse_chemistry_text"),
    "conservation": _load_parser("conservation_parser", "parse_conservation_text"),
    "cs": _load_parser("cs_parser", "parse_cs_text"),
    "econ": _load_parser("econ_parser", "parse_econ_text"),
    "eess": _load_parser("eess_parser", "parse_eess_text"),
    "game_theory": _load_parser("game_theory_parser", "parse_game_theory_text"),
    "geo": parse_geo_text,
...
def main():
    ...
    print("Enter text to parse. Press Ctrl+D (Linux/macOS) or Ctrl+Z (Windows) when done:\n")
    try:
    ...
    if not input_text.strip():
        print("Input text is empty. Please provide text to parse.", file=sys.stderr)
        sys.exit(1)
