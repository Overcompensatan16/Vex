...
opinion_markers_node = next(
    (
        ...
        and any(isinstance(t, ast.Name) and t.id == "OPINION_MARKERS" for t in n.targets)
    ),
    None,
)
if is_opinion_node is None or opinion_markers_node is None:
    raise RuntimeError(f"'is_opinion' or 'OPINION_MARKERS' not found in {FACT_GEN_PATH}")

module = ast.Module(body=[opinion_markers_node, is_opinion_node], type_ignores=[])
global_ns: Dict[str, object] = {}
exec(compile(module, FACT_GEN_PATH, "exec"), global_ns)

_is_opinion_func_obj = global_ns.get("is_opinion")
if not callable(_is_opinion_func_obj):
    raise TypeError(
        f"'is_opinion' from {FACT_GEN_PATH} is not callable: {type(_is_opinion_func_obj)}"
    )
_is_opinion_func: Callable[[str], bool] = _is_opinion_func_obj  # type: ignore[assignment]
